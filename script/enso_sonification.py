...
Converts monthly ENSO Nino 3.4 SST anomaly data into musical sonification
using Javanese gamelan scales (Pelog and Slendro) with spectral analysis.
...
def nino34_to_gamelan_note(value, scale_type='pelog', octave=BASE_OCTAVE):
    ...
    scale_position = normalized * (len(intervals) - 1)
    scale_degree = int(np.round(scale_position))
    scale_degree = np.clip(scale_degree, 0, len(intervals) - 1)
    
    octave_shift = (octave - 4) * 12
...
def calculate_velocity(current_val, previous_val=None, base_velocity=80):
    ...
    if previous_val is not None:
        ...
        change_velocity = int(change * 25)
        mag_velocity = min(127, mag_velocity + change_velocity)
    
    return max(40, min(127, mag_velocity))
